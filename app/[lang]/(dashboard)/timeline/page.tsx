...
export default async function TimelinePage({ params }: Props) {
  ...
  const _locale: Locale = isLocale(lang) ? lang : DEFAULT_LOCALE;
      ...
      <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Real-time visual event log across all LocalStack services
          </p>
        </div>
      </div>

      <TimelineClient initialEvents={initialEvents} />
    </div>
  );
}
